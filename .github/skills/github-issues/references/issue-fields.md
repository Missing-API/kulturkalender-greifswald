...
**Private preview:** Issue fields are currently in private preview. Request access at [https://github.com/orgs/community/discussions/175366](https://github.com/orgs/community/discussions/175366)
...
- **Discover fields** - query the org's issueFields to get field IDs and option IDs
- **Get the issue node ID** - from repository.issue.id
- **Set values** - call setIssueFieldValue with the issue node ID and field entries
- **Batch when possible** - multiple fields can be set in a single mutation call
...
**Warning:** The colon notation (field:Priority:P1) is silently ignored. If using search qualifiers, always use dot notation (field.priority:P1). However, the GraphQL bulk query approach above is more reliable. See [search.md](http://search.md) for the full search guide.
